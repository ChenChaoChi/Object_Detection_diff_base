            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    if save_txt:  # Write to file
                        xywh = (torch.tensor(xyxy)).view(-1).tolist()  # normalized xywh
                        # line = (cls, *xywh, conf)
                        head = osp.basename(img_path)
                        if head not in file:
                            file[head] = {"boxes": [xywh], "labels": [int(cls)], "scores": [float(conf)]}
                        else:
                            file[head]["boxes"].append(xywh)
                            file[head]["labels"].append(int(cls))
                            file[head]["scores"].append(float(conf))
                    # print(file)
        ...
        for i in dd:
            if i != dd[-1]:
                if not os.path.exists(i):
                    os.makedirs(i)
                os.chdir(i)
        with open(dd[-1], 'w', encoding='utf-8') as f:
            json.dump(file, f)
